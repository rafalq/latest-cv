function getUserInfo(user) {
  const githubUsername = document.getElementById('gh-username').innerHTML;
  if (!githubUsername) {
...
function fetchGitHubInfo() {
  const githubUserData = document.getElementById('gh-user-data');
  githubUserData.innerHTML = '';

  const githubRepoData = document.getElementById('gh-repo-data');
  githubRepoData.innerHTML = '';

  const githubUsernameVal = document.getElementById('gh-username').value;
  ...
  Promise.all([
    fetch(`https://api.github.com/users/${githubUsernameVal}`)
      .then((response) => {
      ...
      .then((userData) => (githubUserData.innerHTML = getUserInfo(userData)))
      .catch((error) => console.error(error)),
    fetch(`https://api.github.com/users/${githubUsernameVal}/repos`)
      .then((response) => response.json())
      .then((reposData) => {
        githubRepoData.innerHTML = getReposInfo(reposData);
      }),
  ]);
}

let userInputEl = document.getElementById('gh-username');

userInputEl.addEventListener('keyup', () => fetchGitHubInfo());
